...
def plot_accuracy_comparison(models_data: List[Dict], output_dir: Path):
    ...
    bars1 = ax.bar(x - width/2, train_acc, width, label='Training', color=COLORS['primary'], alpha=0.8)
    bars2 = ax.bar(x + width/2, val_acc, width, label='Validation', color=COLORS['success'], alpha=0.8)

    ax.set_xlabel('Model', fontsize=12, fontweight='bold')
    ax.set_ylabel('Accuracy (%)', fontsize=12, fontweight='bold')
    ax.set_title('Model Accuracy Comparison: Training vs Validation', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.close()
    print(f"{GREEN}✓{NC} Saved: accuracy_comparison.png")


def plot_metrics_comparison(models_data: List[Dict], output_dir: Path):
    ...
    colors_list = [COLORS['primary'], COLORS['success'], COLORS['warning'], COLORS['danger']]

    for i, (metric, label, color) in enumerate(zip(metrics, metric_labels, colors_list)):
        values = [m['training_history']['val_metrics'].get(metric, 0) * 100 for m in models_data]
        offset = width * (i - 1.5)
    ...
    plt.close()
    print(f"{GREEN}✓{NC} Saved: metrics_comparison.png")


def plot_overfitting_analysis(models_data: List[Dict], output_dir: Path):
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    colors_bars = [COLORS['success'] if gap < 5 else COLORS['warning'] if gap < 10 else COLORS['danger'] for gap in gaps]

    bars = ax.bar(model_names, gaps, color=colors_bars, alpha=0.8)

    ax.axhline(y=5, color='green', linestyle='--', linewidth=1, alpha=0.5, label='Good (<5%)')
    ax.axhline(y=10, color='orange', linestyle='--', linewidth=1, alpha=0.5, label='Moderate (5-10%)')

    ax.set_xlabel('Model', fontsize=12, fontweight='bold')
    ax.set_ylabel('Train-Validation Gap (%)', fontsize=12, fontweight='bold')
    ax.set_title('Overfitting Analysis: Train-Validation Gap', fontsize=14, fontweight='bold')
    ax.grid(axis='y', alpha=0.3, linestyle='--')
    ...
    plt.close()
    print(f"{GREEN}✓{NC} Saved: overfitting_analysis.png")


def plot_training_time_comparison(models_data: List[Dict], output_dir: Path):
    fig, ax = plt.subplots(figsize=(10, 6))

    model_names = [m['model_name'] for m in models_data]
    training_times = [m['training_history']['training_time'] for m in models_data]

    bars = ax.barh(model_names, training_times, color=COLORS['secondary'], alpha=0.8)
    ...
    plt.close()
    print(f"{GREEN}✓{NC} Saved: training_time_comparison.png")
